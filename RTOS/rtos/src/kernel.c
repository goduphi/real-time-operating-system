...
bool createThread(_fn fn, const char name[], uint8_t priority, uint32_t stackBytes)
{
    ...
    if (taskCount < MAX_TASKS)
    {
        ...
        if (!found)
        {
            ...
            // I always want to calculate the position of the SRD bits relative to the start of SRAM. Figure out
            // how many 1K regions are required to get the start of the SRD region for a task.
            tcb[i].srd <<= ((uint32_t)tcb[i].spInit - SRAM_BASE - roundUp(stackBytes, 1024)) / 0x400 + 1;
            tcb[i].time = 0;
